...
public class Inventory implements InputProcessor{
	...
	public void display(BitmapFont font, Vector2 screenCoord){
		...
		Global.screenShapes.line(ORIGIN_X + BOX1_WIDTH + BOX2_WIDTH, ORIGIN_Y + HEIGHT - 80, ORIGIN_X + WIDTH, ORIGIN_Y + HEIGHT - 80);
		Global.screenShapes.line(ORIGIN_X + BOX1_WIDTH, ORIGIN_Y, ORIGIN_X + 150, HEIGHT + 250);
		Global.screenShapes.end();

		if(hover != null){
			Global.screen.begin();
			Global.screen.draw(hover.getTexture(), ORIGIN_X + 185, ORIGIN_Y + 100);
			font.draw(Global.screen, hover.getName(), ORIGIN_X + 225, ORIGIN_Y + 120);
			font.draw(Global.screen, "Value: " + hover.getValue(), ORIGIN_X + 170, ORIGIN_Y + 80);
			font.draw(Global.screen, "Weight: " + hover.getWeight(), ORIGIN_X + 235, ORIGIN_Y + 80);
			
			font.draw(Global.screen, "1 - drop", ORIGIN_X + 175, ORIGIN_Y + 50);
			font.draw(Global.screen, "2 - throw", ORIGIN_X + 238, ORIGIN_Y + 50);
			if(hover instanceof Food)
				font.draw(Global.screen, "3 - eat", ORIGIN_X + 175, ORIGIN_Y + 30);
			else if(hover instanceof Wearable || hover instanceof Equipable)
				font.draw(Global.screen, "3 - wear", ORIGIN_X + 175, ORIGIN_Y + 30);
			if(trade instanceof Chest)
				font.draw(Global.screen, "4 - move", ORIGIN_X + 238, ORIGIN_Y + 30);
			else if(trade instanceof Shopkeep)
				font.draw(Global.screen, "4 - sell", ORIGIN_X + 238, ORIGIN_Y + 30);
			Global.screen.end();
		...
		Global.screen.begin();
		font.draw(Global.screen, "    ?????: " + 30, ORIGIN_X + 190, ORIGIN_Y + HEIGHT-15);
		font.draw(Global.screen, " Strength: " + player.getStats().getStrength(), ORIGIN_X + 190, ORIGIN_Y + HEIGHT-35);
		font.draw(Global.screen, "    Health: " + player.getStats().getMaxHitpoints(), ORIGIN_X + 190, ORIGIN_Y + HEIGHT-55);
		font.draw(Global.screen, " Defense: " + player.getStats().getDefense(), ORIGIN_X + 190, ORIGIN_Y + HEIGHT-75);
		font.draw(Global.screen, "Dexterity:  " + player.getStats().getDexterity(), ORIGIN_X + 190, ORIGIN_Y + HEIGHT-95);
		font.draw(Global.screen, "  Points: " + player.getStats().getPoints(), ORIGIN_X + 200, ORIGIN_Y + HEIGHT-115);

		if(player.getStats().getPoints() > 0)
			for(int i=0; i<STATS; i++){
				font.draw(Global.screen, "+", ORIGIN_X + 285, ORIGIN_Y + 205 + i*20);
				pointCollisions[i].setPosition(ORIGIN_X + 285, ORIGIN_Y + 195 + i*20);
			}
		
		font.draw(Global.screen, "Hunger   " + (int)(player.getHunger()*100) + "%", ORIGIN_X + WIDTH - 135, ORIGIN_Y + HEIGHT - 15);
		font.draw(Global.screen, "Wallet       " + wallet, ORIGIN_X + WIDTH - 135, ORIGIN_Y + HEIGHT - 30);
		font.draw(Global.screen, "Weight   " + weight + "/" + 25, ORIGIN_X + WIDTH - 135, ORIGIN_Y + HEIGHT - 45);
		...
		gear.draw(Global.screen);
		
		selector = 'a';
		for(int i=0; i<items.size(); i++, selector++){
			font.draw(Global.screen, selector + ") " + items.get(i).getName(), ORIGIN_X + 10, (ORIGIN_Y + HEIGHT - 10) - i*15);
	...
	public boolean keyDown(int keycode) {
		...
		case Keys.NUM_2:
			if(hover != null){
				...
				player.throwObject.setItem(hover);
				Global.multiplexer.addProcessor(0, player.throwObject);
		...
		case Keys.NUM_3:
			...
			break;
		case Keys.NUM_4:
			if(player.getMenu().equals("Trade") && hover != null){
				AddTradeItemPacket tradeItem = new AddTradeItemPacket();
				tradeItem.ID = trade.getID();
