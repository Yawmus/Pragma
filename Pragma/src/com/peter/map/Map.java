...
public class Map implements MapRenderer{
	...
	public void draw(){
		int originX = (int) (Global.camera.position.x/32 - Gdx.graphics.getWidth()/64) > 0 ? (int) (Global.camera.position.x/32 - Gdx.graphics.getWidth()/64) : 0;
		int originY = (int) (Global.camera.position.y/32 - Gdx.graphics.getHeight()/64) + 2 > 0 ? (int) (Global.camera.position.y/32 - Gdx.graphics.getHeight()/64) + 2 : 0;
		
		if(initialize){
			for(int i=0; i<Play.map.WIDTH; i++)
				for(int j=0; j<Play.map.HEIGHT; j++){
					tiles[i][j] = PacketToObject.tileConverter(initTiles[i][j]);
					if(initTints[i][j] != null && initTints[i][j] != "")
						tiles[i][j].setTint(initTints[i][j]);
				}
			initialize = false;
		}
		for(int x=originX; x<WIDTH && x<(int) (Global.camera.position.x/32 + Gdx.graphics.getWidth()/64) +1; x++)
			for(int y=originY; y<HEIGHT && y<(int) (Global.camera.position.y/32 + Gdx.graphics.getHeight()/64) + 1; y++){
		...
		EntityManager.player.draw(spriteBatch);
		
		weather.draw(spriteBatch);
		
		for(int x=originX; x<WIDTH && x<(int) (Global.camera.position.x/32 + Gdx.graphics.getWidth()/64) + 1; x++)
			for(int y=originY; y<HEIGHT && y<(int) (Global.camera.position.y/32 + Gdx.graphics.getHeight()/64) + 1; y++){
				if(visibleSets.get(floor)[x][y].equals("notVisited"))
